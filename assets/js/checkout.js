jQuery( function( $ ) {
	...
	$(document.body).on('click', '.input-radio', function(){
		if($(this).attr('id') !== 'payment_method_braintree_payment_gateway' ) {
			$('div.payment_method_braintree_payment_gateway').hide();
		}
		else {
			$('div.payment_method_braintree_payment_gateway').show();
		}
	})
	
	$(document.body).on('change', '.existing-card', function(){
		if($(this).val() !== ''){
		$('.cc-table').fadeOut(500);
		$("input[name='save_cc']").prop("checked", false);
		}
		else {
			$('.cc-table').fadeIn(500);
		}
	...
	$(document.body).on('click', '.new-payment-method', function(){
		$('.cc-table').fadeIn(500);
		$('.saved-card:selected').prop('selected', false);
	})
	
$(document.body).on('click', '#braintree-paypal-button', function(){
	$('.saved-card:selected').prop('selected', false);
	$("input[name='save_cc']").prop("checked", false);
})
			...
			execute: function(response){
				if(response.nonce != null){
					$("input[name='payment_method_nonce']").val(response.nonce);
						    $("input[name='payment_type']").val(response.type);
...
/*	var braintree_dropin = {setup: function(){braintree.setup(clientToken, 'dropin', {container: 'dropin-container', 
	...
	var braintree_custom = {setup: function(){braintree.setup(clientToken, 'custom', {id: 'checkout', 
		onPaymentMethodReceived: function(response){
			paymentReceived.execute(response);
	...
	var wc_checkout_form = {
		...
		/*	clientToken = $('#client_token').val();
			var type = $('#payment_form').val();
			if(clientToken){
				
				if(type == 'dropin'){
					braintree_dropin.setup();
							
				}
				else{
					braintree_custom.setup();
